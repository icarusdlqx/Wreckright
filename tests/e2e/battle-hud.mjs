...
export async function runDesktopSupportChecks({ page, check, state, mission, shots }) {
  ...
  const airVisuals = await page.evaluate(() => {
    const scene = globalThis.__ironmuster.engine.renderer.scene;
  ...
  const repairPoint = await page.evaluate((id) => {
    const body = globalThis.__ironmuster.engine.renderer.screenBodyOf(
      globalThis.__ironmuster.world.entities.find((entity) => entity.id === id),
    );
  ...
  await page.waitForFunction(() => globalThis.__ironmuster.useGame.getState().reservesLeft === 0);
  await page.evaluate((resourcePoints) => {
    const { useGame, world } = globalThis.__ironmuster;
    world.resources.set(useGame.getState().playerTeam, resourcePoints);
    useGame.getState().patch({ resourcePoints });
