...
const EXPLANATIONS = [
  ...
  ['Called Shot', 'Choose a body section, then a visible enemy. Disabling legs can preserve a hull when the mission can be won by holding ground.'],
...
export function ActiveOrderHelp({ mode }: { mode: OrderMode }) {
  ...
  const text = mode === 'attack' ? 'Choose a visible hostile. Your machine will approach a firing position.'
    : mode === 'called_shot' ? 'Choose a body section, then a visible hostile. Hold fire once the desired damage is done.'
