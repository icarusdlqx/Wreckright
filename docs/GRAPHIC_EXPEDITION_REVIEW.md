...
| Stores and yard | Equipment stock, trade, hull rebuilding and faction supply limitations affect what can be fielded. These should remain reachable without competing with the current contract. | Existing panels are grouped within Stores & yard. Further prioritisation of useful purchases is future work. |
...
| Battle | Several useful readouts already exist: objectives, component damage, orders, fire modes, support, sensor information and training prompts. Their visibility must survive brighter scenery and a larger typographic hierarchy. | Shared dark tactical surfaces, clearer type and state styling accompany the brighter battlefield palette. Browser coverage exercises representative day/night fields, camera distances and effects modes. |
| Results and salvage | The debrief already leads with the payout and salvage receipt and keeps detailed recovery/selection in a disclosure. It also records pilot consequences and employer history. | The concise receipt and its adjustment/finalisation flow receive the shared paper styling. No new recovery or reward rules are introduced. |
| Files and restart | Save/load/export/import, campaign choice and restart were peers in a crowded header. Immediate restart sat close to routine controls. Recovery notices need to remain conspicuous. | Company files groups file actions, and a cancel-first dialog confirms restart. Import/export and memory-only recovery behaviour remain intact. |
...
- Fast suite: **312 files, 2,536 tests passed**, including unchanged deterministic combat/campaign tests. No sim-affecting code or data changed, so the separate balance gate was not required.
...
- Before/after inspection covers home, briefing, battle, campaign, workshop/refit, debrief, night combat and terrain wear. Touch captures cover portrait, landscape and tablet layouts. The final validation record links the inspected captures.
...
- Final touch review also resolved an inherited header-width rule, restored the scoped campaign cartography and kept the debrief exit visible while scrolling. Focused campaign probes confirmed no horizontal overflow, overlapping cards or misplaced route endpoints at 390px, 844px and 1024px touch widths. Lane markings now use a separate pale pigment to retain contrast with the road.
...
The heat-warning audio fixture now observes the warning without advancing a random thermal roll, then clears friendly heat before the hidden-hostile privacy check. A native seeded reproduction showed that the previous extra source was a legitimate friendly shutdown. The exact three-warning and zero-hidden-source assertions remain; production audio and simulation code are unchanged.
